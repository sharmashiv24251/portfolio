const contributionsUrl =
  "https://github.com/users/sharmashiv24251/contributions";
const profileUrl = "https://github.com/sharmashiv24251";

type ContributionDay = {
  date: string;
  level: 0 | 1 | 2 | 3 | 4;
  title: string;
};

const shipSvg = `<?xml version="1.0" encoding="utf-8"?><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 113.74 122.88" style="enable-background:new 0 0 113.74 122.88" xml:space="preserve"><style type="text/css">.st0{fill-rule:evenodd;clip-rule:evenodd;}</style><g><path class="st0" d="M11.98,76.68c-1.28-2.62-0.71-4.04,1.52-4.4l38.95,0.05l0-0.06V10.68c-10.63,18.33-4.47,36.89-3.21,46.21 c0.2,1.51,1.09,4.11-0.75,4.7l-24.47,9.79c-0.85,0.27-1.59,0.24-2.21-0.09c-1.22-0.65-2.31-3.45-2.9-4.75 C8.6,43.61,17.96,25.91,39.98,11.6c2.99-1.95,9.02-5.81,12.46-6.38v-2.5c0-1.5,1.22-2.71,2.71-2.71c1.5,0,2.71,1.22,2.71,2.71 v69.56l0,0.07l47.12,0.06c2.47-0.13,3.2,2.17,1.46,3.56c-5.81,4.5-13.77,8.89-22.68,12.34c-3.41-0.18-6.7-0.97-9.38-2.32 c-2.42-1.22-4.31-2.89-5.25-4.97l0,0c-0.12-0.27-0.31-0.52-0.57-0.71c-0.8-0.6-1.94-0.43-2.53,0.38c-2.31,3.1-4.66,5.27-6.96,6.54 c-1.76,0.97-3.47,1.4-5.1,1.31c-1.61-0.09-3.18-0.7-4.67-1.8c-1.72-1.28-3.32-3.22-4.75-5.8c-0.18-0.36-0.49-0.65-0.88-0.83 c-0.92-0.4-1.99,0.01-2.39,0.93c-1.14,2.59-3.11,4.43-5.5,5.61c-2.26,1.11-4.9,1.64-7.58,1.62c-2.69-0.01-5.4-0.58-7.8-1.64 c-0.08-0.03-0.15-0.07-0.23-0.1C16.97,83.91,14.2,80.66,11.98,76.68L11.98,76.68z M1.81,108.43c-0.95,0.05-1.76-0.68-1.81-1.64 c-0.05-0.95,0.68-1.76,1.64-1.81c2.32-0.13,4.03-0.83,5.39-2.01c1.44-1.25,2.59-3.09,3.71-5.41c0.42-0.86,1.45-1.22,2.31-0.8 c0.4,0.2,0.7,0.53,0.85,0.92c1.35,2.68,3.62,4.65,6.25,5.81c1.75,0.77,3.64,1.18,5.51,1.18c1.86,0,3.7-0.39,5.36-1.2 c2.21-1.09,4.11-2.95,5.31-5.69c0.38-0.87,1.41-1.27,2.28-0.89c0.37,0.16,0.66,0.45,0.84,0.78c0.93,1.67,2.18,3.06,3.65,4.16 c2.56,1.91,5.79,2.95,9.17,3.09c3.41,0.14,6.95-0.64,10.1-2.37c2.22-1.22,4.23-2.92,5.86-5.1c0.57-0.77,1.65-0.93,2.41-0.36 c0.25,0.18,0.43,0.42,0.55,0.68l0,0c1.05,2.32,2.96,4.1,5.3,5.28c2.13,1.07,4.62,1.65,7.15,1.71c2.54,0.05,5.11-0.43,7.43-1.48 c2.75-1.24,5.13-3.3,6.6-6.22c0.43-0.85,1.47-1.19,2.32-0.76c0.4,0.2,0.68,0.53,0.83,0.91l0,0c1.09,2.76,2.67,4.82,4.57,6.09 c1.85,1.23,4.04,1.72,6.38,1.38c0.94-0.14,1.82,0.52,1.96,1.46c0.14,0.94-0.52,1.82-1.46,1.96c-3.2,0.47-6.22-0.21-8.8-1.93 c-1.73-1.15-3.24-2.76-4.45-4.8c-1.76,2.25-4.02,3.93-6.53,5.06c-2.78,1.26-5.88,1.84-8.92,1.78c-3.04-0.06-6.05-0.76-8.64-2.07 c-2.22-1.12-4.15-2.68-5.58-4.66c-1.59,1.68-3.39,3.05-5.32,4.12c-3.7,2.04-7.88,2.96-11.9,2.79c-4.05-0.17-7.96-1.44-11.09-3.76 c-1.1-0.82-2.11-1.77-2.99-2.85c-1.48,2.16-3.39,3.75-5.53,4.8c-2.14,1.05-4.5,1.56-6.87,1.55c-2.36,0-4.73-0.51-6.9-1.46 c-2.44-1.08-4.65-2.74-6.34-4.91c-0.91,1.51-1.92,2.79-3.13,3.84C7.35,107.27,4.98,108.26,1.81,108.43L1.81,108.43z M1.81,122.88 c-0.95,0.05-1.76-0.68-1.81-1.64c-0.05-0.95,0.68-1.76,1.64-1.81c2.32-0.13,4.03-0.83,5.39-2.01c1.44-1.25,2.59-3.09,3.71-5.41 c0.42-0.86,1.45-1.22,2.31-0.8c0.4,0.2,0.7,0.53,0.85,0.92c1.35,2.68,3.62,4.65,6.25,5.81c1.75,0.77,3.64,1.18,5.51,1.18 c1.86,0,3.7-0.39,5.36-1.2c2.21-1.09,4.11-2.95,5.31-5.69c0.38-0.87,1.41-1.27,2.28-0.89c0.37,0.16,0.66,0.45,0.84,0.78 c0.93,1.67,2.18,3.06,3.65,4.16c2.56,1.91,5.79,2.95,9.17,3.09c3.41,0.14,6.95-0.64,10.1-2.37c2.22-1.22,4.23-2.92,5.86-5.1 c0.57-0.77,1.65-0.93,2.41-0.36c0.25,0.18,0.43,0.42,0.55,0.68l0,0c1.05,2.32,2.96,4.1,5.3,5.28c2.13,1.07,4.62,1.65,7.15,1.71 c2.54,0.05,5.11-0.43,7.43-1.48c2.75-1.24,5.13-3.3,6.6-6.22c0.43-0.85,1.47-1.19,2.32-0.76c0.4,0.2,0.68,0.53,0.83,0.91l0,0 c1.09,2.76,2.67,4.82,4.57,6.09c1.85,1.23,4.04,1.72,6.38,1.38c0.94-0.14,1.82,0.52,1.96,1.46c0.14,0.94-0.52,1.82-1.46,1.96 c-3.2,0.47-6.22-0.21-8.8-1.93c-1.73-1.15-3.24-2.76-4.45-4.8c-1.76,2.25-4.02,3.93-6.53,5.06c-2.78,1.26-5.88,1.84-8.92,1.78 c-3.05-0.06-6.05-0.76-8.64-2.07c-2.22-1.12-4.15-2.68-5.58-4.66c-1.59,1.68-3.39,3.05-5.32,4.12c-3.7,2.04-7.88,2.96-11.9,2.79 c-4.05-0.17-7.96-1.44-11.09-3.76c-1.1-0.82-2.11-1.77-2.99-2.85c-1.48,2.16-3.39,3.75-5.53,4.8c-2.14,1.05-4.5,1.56-6.87,1.55 c-2.36,0-4.73-0.51-6.9-1.46c-2.44-1.08-4.65-2.74-6.34-4.91c-0.92,1.51-1.92,2.79-3.13,3.84C7.35,121.71,4.98,122.71,1.81,122.88 L1.81,122.88z M76.73,90.77c-15.92,5-33.72,6.69-47.46,1.08c2.84-0.12,5.63-0.76,8.09-1.97c2.2-1.08,4.14-2.61,5.62-4.62 c1.28,1.81,2.67,3.27,4.15,4.37c2.06,1.54,4.29,2.38,6.63,2.51c2.31,0.13,4.69-0.45,7.05-1.75c2.18-1.2,4.35-3.03,6.47-5.5 c1.38,1.76,3.26,3.21,5.47,4.32C73.98,89.84,75.32,90.36,76.73,90.77L76.73,90.77z"/></g></svg>`;
const displayShipSvg = shipSvg
  .replace(/^<\?xml[^>]*>/, "")
  .replace("<svg ", '<svg aria-hidden="true" class="h-full w-full fill-current" ');

const levelClasses = {
  0: "bg-stone-200 dark:border-white/10 dark:bg-[#3a3a3a]",
  1: "bg-emerald-200 dark:bg-emerald-900",
  2: "bg-emerald-400 dark:bg-emerald-800",
  3: "bg-emerald-600 dark:bg-emerald-600",
  4: "bg-emerald-800 dark:bg-emerald-400",
} satisfies Record<ContributionDay["level"], string>;

export async function ContributionsSection() {
  const contributions = await getContributions();

  return (
    <section className="mb-12 scroll-mt-24 2xl:mb-16" id="shipping">
      <div className="mb-6 flex flex-col gap-4 border-t border-stone-950 pt-10 dark:border-[#333333] sm:mb-8 sm:flex-row sm:items-end sm:justify-between 2xl:mb-10 2xl:pt-14">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-3">
          <h2 className="font-serif text-3xl font-bold leading-tight text-stone-950 dark:text-white sm:text-4xl 2xl:text-[4rem]">
            ship. ship. ship.
          </h2>
          <span
            className="inline-block size-11 text-stone-950 dark:text-white sm:size-12 2xl:size-16"
            dangerouslySetInnerHTML={{ __html: displayShipSvg }}
          />
        </div>

        <a
          className="w-fit border border-stone-950 px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-stone-950 transition-colors hover:bg-stone-100 dark:border-[#333333] dark:text-white dark:hover:bg-[#242424]"
          href={profileUrl}
          rel="noreferrer"
          target="_blank"
        >
          GitHub profile
        </a>
      </div>

      <div className="border border-stone-950 bg-white p-4 dark:border-[#333333] dark:bg-[#242424] sm:p-6 2xl:p-8">
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between">
          <p className="font-serif text-lg font-semibold text-stone-950 dark:text-white 2xl:text-2xl">
            {contributions.summary}
          </p>
          <p className="font-mono text-xs uppercase tracking-[0.12em] text-stone-500 dark:text-[#a0a0a0]">
            GitHub activity
          </p>
        </div>

        {contributions.days.length > 0 ? (
          <>
            <div className="overflow-x-hidden pb-2">
              <div
                aria-label={contributions.summary}
                className="grid min-w-[46rem] grid-flow-col grid-rows-7 gap-1 sm:min-w-[52rem] sm:gap-1.5 xl:w-full 2xl:gap-2"
                role="img"
              >
                {contributions.days.map((day) => (
                  <span
                    aria-label={day.title}
                    className={`aspect-square w-full border border-stone-950/5 shadow-sm shadow-transparent dark:shadow-black/10 ${levelClasses[day.level]}`}
                    key={day.date}
                    title={day.title}
                  />
                ))}
              </div>
            </div>

            <div className="mt-5 flex items-center gap-2 text-[11px] text-stone-500 dark:text-[#a0a0a0]">
              <span>Less</span>
              {[0, 1, 2, 3, 4].map((level) => (
                <span
                  className={`size-3 border border-stone-950/5 ${levelClasses[level as ContributionDay["level"]]}`}
                  key={level}
                />
              ))}
              <span>More</span>
            </div>
          </>
        ) : (
          <p className="leading-relaxed text-stone-600 dark:text-[#a0a0a0]">
            Could not load the live contribution graph. Open GitHub profile to
            see the latest activity.
          </p>
        )}
      </div>
    </section>
  );
}

async function getContributions() {
  try {
    const response = await fetch(contributionsUrl, {
      next: { revalidate: 60 * 60 * 12 },
    });

    if (!response.ok) {
      throw new Error(`GitHub returned ${response.status}`);
    }

    const html = await response.text();
    return parseContributions(html);
  } catch {
    return {
      summary: "GitHub contributions",
      days: [] as ContributionDay[],
    };
  }
}

function parseContributions(html: string) {
  const summary =
    html
      .match(
        /<h2[^>]*id="js-contribution-activity-description"[^>]*>([\s\S]*?)<\/h2>/,
      )?.[1]
      ?.replace(/<[^>]*>/g, " ")
      .replace(/\s+/g, " ")
      .trim() || "GitHub contributions";

  const tooltipById = new Map<string, string>();

  for (const tooltip of html.matchAll(
    /<tool-tip[^>]*for="([^"]+)"[^>]*>([\s\S]*?)<\/tool-tip>/g,
  )) {
    tooltipById.set(
      tooltip[1],
      decodeHtml(tooltip[2].replace(/<[^>]*>/g, " ").trim()),
    );
  }

  const days: ContributionDay[] = [];

  for (const cell of html.matchAll(
    /<td[^>]*class="ContributionCalendar-day"[^>]*><\/td>/g,
  )) {
    const markup = cell[0];
    const date = getAttribute(markup, "data-date");
    const level = Number(
      getAttribute(markup, "data-level"),
    ) as ContributionDay["level"];
    const id = getAttribute(markup, "id");

    if (!date || !Number.isInteger(level) || level < 0 || level > 4) {
      continue;
    }

    days.push({
      date,
      level,
      title: tooltipById.get(id) || date,
    });
  }

  return {
    summary,
    days: days.sort((a, b) => a.date.localeCompare(b.date)),
  };
}

function getAttribute(markup: string, name: string) {
  return markup.match(new RegExp(`${name}="([^"]*)"`))?.[1] || "";
}

function decodeHtml(value: string) {
  return value
    .replaceAll("&quot;", '"')
    .replaceAll("&#39;", "'")
    .replaceAll("&amp;", "&")
    .replaceAll("&lt;", "<")
    .replaceAll("&gt;", ">");
}
